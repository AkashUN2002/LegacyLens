...
def ensure_indexes(db: Database) -> None:
    ...
    # Self-heal: older versions created a GLOBAL unique index named
    # "entity_id_1". It conflicts with the indexes we create below, so drop it
    # if present. (Migration is otherwise handled by migrate_indexes.py.)
    _drop_index_if_exists(entities, "entity_id_1")

    # Per-repo uniqueness (repo_id + entity_id) so multiple repos coexist;
    # two repos may share an entity_id like "main.py::main::1".
    entities.create_index([("repo_id", 1), ("entity_id", 1)], unique=True)
    ...
    edges = db[EDGES]
    # Self-heal: older versions created a GLOBAL unique index on from_id+to_id.
    _drop_index_if_exists(edges, "from_id_1_to_id_1")
    edges.create_index("from_id")
    edges.create_index("to_id")
    edges.create_index("repo_id")
    # Per-repo uniqueness for edges too.
    edges.create_index([("repo_id", 1), ("from_id", 1), ("to_id", 1)], unique=True)
...
def _drop_index_if_exists(collection, index_name: str) -> None:
    ...
    try:
        ...
        if index_name in existing:
            ...
            print(f"[db] dropped legacy index {collection.name}.{index_name}")
    except Exception as exc:
        print(f"[db] could not check/drop index {index_name}: {exc}")
